...
class MatrixPipeExtensions(pipe : Pipe) {
  def toMatrix[RowT,ColT,ValT](fields : Fields)
    (implicit conv : TupleConverter[(RowT,ColT,ValT)], setter : TupleSetter[(RowT,ColT,ValT)], tracking : Tracking) = {
    val matPipe = RichPipe(pipe).mapTo(fields -> ('row,'col,'val))((tup : (RowT,ColT,ValT)) => tup)(conv,setter,tracking)
    new Matrix[RowT,ColT,ValT]('row, 'col, 'val, matPipe)
  }
  def mapToMatrix[T,RowT,ColT,ValT](fields : Fields)(mapfn : T => (RowT,ColT,ValT))
    (implicit conv : TupleConverter[T], setter : TupleSetter[(RowT,ColT,ValT)], tracking : Tracking) = {
    val matPipe = RichPipe(pipe).mapTo(fields -> ('row,'col,'val))(mapfn)(conv,setter,tracking)
    new Matrix[RowT,ColT,ValT]('row, 'col, 'val, matPipe)
  }
  def flatMapToMatrix[T,RowT,ColT,ValT](fields : Fields)(flatMapfn : T => Iterable[(RowT,ColT,ValT)])
    (implicit conv : TupleConverter[T], setter : TupleSetter[(RowT,ColT,ValT)], tracking : Tracking) = {
    val matPipe = RichPipe(pipe).flatMapTo(fields -> ('row,'col,'val))(flatMapfn)(conv,setter,tracking)
    new Matrix[RowT,ColT,ValT]('row, 'col, 'val, matPipe)
  }
}
